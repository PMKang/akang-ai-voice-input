...
还有一个底层改动：不管网络好坏，音频上传都按录音顺序排队。

录音不是攒成一整个文件再发出去，而是不断切成很短的音频片段上传。现在这些片段始终按录制顺序排队，前一段发完，下一段再发。正常网络下这只是把链路收紧；网络一差时，它能避免客户端自己的并发发送再增加不确定性。
