...
4. 生成至少 32 位随机上报令牌，并把它保存为 Worker Secret：
   ...
   第二条命令会提示输入值，请粘贴第一条命令生成的令牌。部署后还要将同一令牌填进 Noboard 开发者选项，它只保存在当前 Mac 的 Keychain。
...
8. 打开 Noboard 设置的开发者模式，填入同一枚上报令牌；开启“自动发送脱敏崩溃报告”，再点“发送测试告警”。只有同时看到应用显示 Worker 已接收、D1 新增测试记录、飞书收到“链路测试”，才算端到端验证通过。
...
上报令牌不是设备证明：Keychain 保护的是本机静态存储，公开发行的客户端仍可能被提取令牌。当前实现定位为个人/受控测试版本；公开发行前应升级为每安装注册、可撤销和可轮换的凭证体系，不能把一枚共享令牌当作最终安全边界。
