...
| --- | --- | --- | --- |
| macOS | `1.11.0` | macOS 12 Monterey or later; Apple silicon and Intel | Repository root: `Sources/`, `Package.swift`, and `AkangVoiceInput.xcodeproj` |
...
`1.11.0` update is macOS-only; Windows remains on `1.7.0`.
...
- Diagnostic reports exclude keys, Workspace IDs, audio, and transcription text. The controlled macOS build automatically sends only redacted crash summaries; users do not configure an upload credential.
